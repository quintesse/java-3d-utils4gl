 ...
 */
public class QuadArrayGeometry extends GeometryBase {
	PolygonIterator m_polyIter;
	
	/**
	 * Creates a QuadArrayGeometry using the given VertexBuffer for its vertex data.
	 * The vertex data is assumed to consist of 4 vertices for each quad. See the
	 ...
	 */
	public QuadArrayGeometry(VertexBuffer _buffer) {
		super(_buffer);
	...
	private class PolygonIterator implements Iterator {
		...
		public PolygonIterator(int _nFormat) {
			m_polygon = new Polygon(4, _nFormat);
			m_nIndex = 0;
		}
		
		public boolean hasNext() {
			return ((getBuffer().getSize() - m_nIndex) >= 4);
		}
